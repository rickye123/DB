...
public class Table {
    ...
    public boolean readFromFileToTable(String filename, Table tab) {

        Upload file = new Upload(); 

        if(table.isEmpty()) {
            if(file.readFromFile(filename, tab) == false) {
                return false; 
            }
            return true; 
        }
    ...
    public void writeToFileFromTable(String filename, Table tab) {

    	Upload file = new Upload(); 

    	file.writeToFile(filename, tab, table);
    }
