...
public class Print {
    ...
    public void printTablesInDatabase(Database db, String ... tableNames) {
        ...
        int[] keys = getKeys(db, tableNames);
        int[] size = getSize(db, tableNames); 
        
        printDashes(dash);
        System.out.printf("|%-30s|%-10s|%-13s|\n", "Tables", "KeyCol", "NumOfRecords");
        printDashes(dash);
        for (int i = 0; i < tables.size(); i++) {
            ...
            System.out.printf("%-13s|", size[i]);
            System.out.println();
            printDashes(dash);
        }
    }

    // return the key columns for each table
    private int[] getKeys(Database db, String ... tableNames) {
 
        int[] keys = new int[tableNames.length];

        int i = 0; 
        for (String s : tableNames) {
            keys[i] = db.getKeyCol(i);
            i++; 
        }
        return keys; 
    }

    // return the size (number of records) of each table 
    private int[] getSize(Database db, String ... tableNames) {
        int[] size = new int[tableNames.length]; 

        int i = 0; 
        for (String s : tableNames) {
            size[i] = db.getDatabaseTable(i).size(); 
            i++; 
        }
        return size;
    }
